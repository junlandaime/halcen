                    ...
                    <tbody class="bg-white divide-y">
                        @foreach ($abouts as $about)
                            <tr class="text-gray-700">
                                        ...
                                        @if ($about->hero_image)
                                            <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                                <img class="object-cover w-full h-full rounded-full"
                                                    src="{{ Storage::url($about->hero_image) }}" alt="{{ $about->title }}">
                                            </div>
                                        @endif
                                        <div>
                                            <p class="font-semibold">{{ $about->title }}</p>
                                            <p class="text-xs text-gray-600">{{ $about->hero_title }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    <p class="text-gray-600">Slug: {{ $about->slug }}</p>
                                    <p class="text-gray-600">Urutan: {{ $about->order }}</p>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    <span
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <p class="text-gray-600">Sections: {{ $about->sections->count() }}</p>
                                    <p class="text-gray-600">Teams: {{ $about->teams->count() }}</p>
                                    <p class="text-gray-600">Programs: {{ $about->programs->count() }}</p>
                                </td>
                                <td class="px-4 py-3 text-sm">
