                ...
                @foreach ($abouts as $about)
                    <a href="{{ route('abouts.show', $about->slug) }}" class="group">
                        <div class="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 group-hover:-translate-y-2"
                            ...
                            <div class="p-6">
                                <h2
                                    class="text-2xl font-bold text-primary mb-4 group-hover:text-primary-dark transition-colors duration-300">
                                    {{ $about->title }}</h2>
                                <p class="text-gray-600 mb-4">{{ Str::limit($about->description, 150) }}</p>
                                <div
                                    class="flex items-center text-primary group-hover:text-primary-dark transition-colors duration-300">
                                    <span>Baca Selengkapnya</span>
                                    <svg class="w-5 h-5 ml-2 transition-transform duration-300 group-hover:translate-x-2"
